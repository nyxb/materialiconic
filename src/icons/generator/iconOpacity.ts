 ...
 */
export function setIconOpacity(options: IconJsonOptions,
   fileNames?: string[]) {
 ...
 */
export function validateOpacityValue(opacity: number | undefined) {
 ...
 * @param opacity Opacity value.
 */
function addOpacityAttribute(svgRoot: string, opacity: number) {
 ...
 * @param svgRoot Root element of the SVG icon.
 */
function removeOpacityAttribute(svgRoot: string) {
   const pattern = new RegExp(/\sopacity="[\d.]+"/)
   return svgRoot.replace(pattern, '')
}

function adjustOpacity(iconPath: string,
