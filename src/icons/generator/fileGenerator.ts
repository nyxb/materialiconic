 ...
 */
export function loadFileIconDefinitions(fileIcons: FileIcons,
 ...
 */
function mapSpecificFileIcons(icon: FileIcon,
   ...
   iconMappingType.forEach((name) => {
      ...
      const shouldOverwriteFileNames = Object.keys(customFileAssociation).some(
         (key) => {
            ...
            return name.toLowerCase().includes(fileExtension.toLowerCase())
         },
      )
 ...
 */
function disableIconsByPack(fileIcons: FileIcons,
   activatedIconPack: string): FileIcon[] {
...
function setIconDefinition(config: IconConfiguration,
   iconName: string,
   appendix = '') {
...
export function generateFileIcons(color: string | undefined) {
...
function getCustomIcons(fileAssociations: IconAssociations | undefined) {
   if (!fileAssociations)
      return []
