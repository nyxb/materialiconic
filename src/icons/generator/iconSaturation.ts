 ...
 */
export function setIconSaturation(options: IconJsonOptions,
   fileNames?: string[]) {
 ...
 */
function getSVGRootElement(svg: string) {
 ...
 */
function addFilterAttribute(svgRoot: string) {
 ...
 * @param svgRoot Root element of the SVG icon.
 */
function removeFilterAttribute(svgRoot: string) {
   const pattern = new RegExp(/\sfilter="[^"]+?"/)
 ...
 * @param svg SVG file as string.
 */
function addFilterElement(svg: string, value: number) {
 ...
 * @param svg SVG file as string.
 */
function removeFilterElement(svg: string) {
   const pattern = new RegExp(/<filter id="saturation".+<\/filter>(.*<\/svg>)/)
 ...
 */
export function validateSaturationValue(saturation: number | undefined) {
   return saturation !== undefined && saturation <= 1 && saturation >= 0
}

function adjustSaturation(iconsPath: any,
   options: IconJsonOptions): ((value: string, index: number, array: string[]) => void) {
   return (iconFileName) => {
