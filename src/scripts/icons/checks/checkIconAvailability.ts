 ...
 */
function fsReadAllIconFiles(err: NodeJS.ErrnoException | null,
   files: string[]) {
   if (err)
...
function isIconAvailable(icon: FileIcon | FolderIcon | DefaultIcon,
   iconType: IconType,
   iconColor: IconColor,
   ...
   if (
    ...
    && !wrongIconNames[iconType].includes(iconName)
   )
...
function getAllFolderIcons(theme: FolderTheme) {
...
function logIconInformation(wrongIcons: string[], title: string) {
